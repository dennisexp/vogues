...
const logsConfig = require('../config/log4_cfg.js');
//加载配置文件
...
let formatText = {
    info: function(info) {
        ...
        logText += "\n" + "***************info log start ***************" + "\n";
        //响应内容
        logText += "content: " + JSON.stringify(info) + "\n";
        //响应日志结束信息
    ...
    handle: function(info) {
        ...
        logText += "\n" + "*************** handle log start ***************" + "\n";
        //响应内容
        logText += "content: " + JSON.stringify(info).replace(/\\n/g, "\n") + "\n";
        //响应日志结束
...
module.exports = {
    ...
    logError: function(ctx, error, resTime) {
        if (ctx && error) {
            consoleLogger.info(formatText.error(ctx, error, resTime));
